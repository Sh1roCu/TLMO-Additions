...
public class BlockUpScheduleBehavior extends Behavior<EntityMaid> {
    public BlockUpScheduleBehavior() {
        super(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT, InitEntities.TARGET_POS, MemoryStatus.VALUE_ABSENT));
    }
    ...
    protected void start(ServerLevel p_22540_, EntityMaid maid, long p_22542_) {
        ...
        if (context.hasTarget()) {
            if (context.getStatus() != BlockUpContext.STATUS.UP && !context.isOnLine(maid.blockPosition())) {
                ...
                maid.getBrain().setMemory(InitEntities.TARGET_POS, new BlockPosTracker(context.getTargetPos()));
            ...
            if (targetPosBlockUp != null) {
                ...
                maid.getBrain().setMemory(InitEntities.TARGET_POS, new BlockPosTracker(targetPosBlockUp.getA()));
