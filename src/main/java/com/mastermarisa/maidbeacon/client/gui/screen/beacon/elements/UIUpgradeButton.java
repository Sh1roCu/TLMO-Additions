...
public class UIUpgradeButton extends UIButton {
    ...
    private boolean unlocked() {
        ...
        boolean result = index == -1 || (itemInHand.is(ModItems.MOBILE_BEACON)
                && MobileBeaconData.getOrDefault(itemInHand).getLevel() > index);
    ...
    public void trigger(BeaconScreen screen) {
        ...
        if (MaidUtils.count(playerItemHandler, StackPredicate.of(itemStack.getItem())) >= itemStack.getCount()) {
            UpgradeBeaconMessage message = new UpgradeBeaconMessage(index, hand == InteractionHand.MAIN_HAND ? 0 : 1, player.getUUID());
            ClientPlayNetworking.send(UpgradeBeaconMessage.ID, UpgradeBeaconMessage.encode(message));
            ItemStack itemInHand = player.getItemInHand(hand);
