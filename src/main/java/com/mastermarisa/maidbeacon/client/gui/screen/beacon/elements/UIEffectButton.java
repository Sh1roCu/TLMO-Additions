...
public class UIEffectButton extends UIButton {
    ...
    public UIEffectButton(MobileBeaconData mobileBeaconData, EffectAura effectAura, UUID uuid, InteractionHand hand) {
        ...
        tooltip.add(effectAura.getEffect().getDisplayName().copy().append(Component.literal(" " + EncodeUtils.toRoman(effectAura.effectLevel + 1))));
    ...
    public void trigger(UUID uuid, InteractionHand hand) {
        if (toggled) {
            OperateEffectMessage message = new OperateEffectMessage(0, index, hand == InteractionHand.MAIN_HAND ? 0 : 1, uuid);
            ...
            OperateEffectMessage message = new OperateEffectMessage(1, index, hand == InteractionHand.MAIN_HAND ? 0 : 1, uuid);
            ClientPlayNetworking.send(OperateEffectMessage.ID, OperateEffectMessage.encode(message));
